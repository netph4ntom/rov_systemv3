...
# MJPEG streaming server untuk kamera bawah.
...
from config import PORT_STREAM_BOTTOM, MJPEG_QUALITY, FRAME_FPS
...
# Queue references (di-set oleh run_bottom_stream_server)
...
def _capture_loop():
    ...
    while True:
        ...
        # Jalankan deteksi QR sebelum overlay, agar bbox/status bisa ditampilkan
...
def _handle_commands():
    ...
    logger.info(f"[BottomStream] Command diterima: {action}")
...
def _send_result(action: str, filepath:Optional[str]):
    ...
    try:
        ...
        logger.warning("[BottomStream] result_queue penuh, result dibuang")
...
if __name__ == "__main__":
    ...
    run_bottom_stream_server(mgr.Queue(), mgr.Queue(), mgr.Queue(), mgr.Queue())