# camera_bottom/record.py
# VideoWriter wrapper untuk merekam stream kamera bawah ke file .mp4
#
# Lifecycle:
...
class BottomRecorder:
    ...
    def start(self, filename:Optional[str] = None) -> str:
        ...
        if self.is_recording:
            logger.warning("[BottomRecorder] Sudah recording, stop dulu...")
            self.stop()
        ...
        if filename is None:
            ...
            filename = f"bottom_{timestamp}.mp4"
        ...
        if not self._writer.isOpened():
            logger.error(f"[BottomRecorder] Gagal buka VideoWriter: {self._filepath}")
            self._writer = None
            self._filepath = None
            return ""

        self.is_recording = True
        logger.info(f"[BottomRecorder] Recording dimulai → {self._filepath}")
    ...
    def stop(self) ->Optional[str]:
        ...
        self._filepath = None
        logger.info(f"[BottomRecorder] Recording selesai → {saved_path}")
        return saved_path
