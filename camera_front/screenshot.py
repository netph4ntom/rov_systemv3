# camera_front/screenshot.py
# Screenshot handler untuk kamera depan.
...
class FrontScreenshot:
    """Screenshot taker untuk kamera depan."""
    ...
    def take(self, frame, filename:Optional[str] = None) ->Optional[str]:
        ...
        """
        if frame is None:
            logger.warning("[FrontScreenshot] Frame kosong, skip screenshot")
            return None
        ...
        if success:
            logger.info(f"[FrontScreenshot] Disimpan → {filepath}")
            return filepath
        else:
            logger.error(f"[FrontScreenshot] Gagal menyimpan → {filepath}")
            return None